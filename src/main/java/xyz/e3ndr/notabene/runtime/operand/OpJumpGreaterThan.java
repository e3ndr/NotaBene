...
public class OpJumpGreaterThan implements NBOperand {
    ...
    public void execute(NBRuntime runtime) throws ExecutionException {
        ...
        if (regAVal > regBVal) {
            runtime
                .getRuntimeLogger()
                .log(LogLevel.TRACE, String.format("[OpJumpGreaterThan] Jumping to [%d]", newAddress));
    ...
    public String getDebugInfo(NBRuntime runtime) throws ExecutionException {
        return String.format("$jgt #%d #%d [%d]", runtime.peek(), runtime.peek(), runtime.peek());
    }
