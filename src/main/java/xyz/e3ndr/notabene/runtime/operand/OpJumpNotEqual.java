...
public class OpJumpNotEqual implements NBOperand {
    ...
    public void execute(NBRuntime runtime) throws ExecutionException {
        ...
        if (regAVal != regBVal) {
            runtime
                .getRuntimeLogger()
                .log(LogLevel.TRACE, String.format("[OpNotJumpEqual] Jumping to [%d]", newAddress));
    ...
    public String getDebugInfo(NBRuntime runtime) throws ExecutionException {
        return String.format("$jne #%d #%d [%d]", runtime.peek(), runtime.peek(), runtime.peek());
    }
